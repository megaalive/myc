...
| compile             | (always)   | gcc                 | `-Werror` clean under `-O2 -Wall -Wextra -pedantic` + memory tier (`-Warray-bounds`, `-Wstringop-overflow`, `-Wuse-after-free`, `-Wfree-nonheap-object`, `-Wformat-overflow`, …) | **hard gate**; gcc major < 9 is skipped when a newer PATH entry exists; use `--gcc PATH` or `MYC_GCC` to override, and an unusable/old toolchain is reported as `UNAVAILABLE` with a diagnostic |
...
| limits             | `myc limits [--json]` | resource ceiling registry (PR-018, P7-T01) | **resource ceilings truth table**: 29 entries (max_source/stdin/backend_output/output_cap/timeout/diagnostics/gates/evidence/debt/contract/rel/driver/filc/assumptions/sm/rsrc/units/coaching/witness/divergence/matrix/arena) dicantumkan dengan nilai + kelas enforcement **HARD** (ingress fail-fast) / **soft** (cap + debt `MYC-INCOMPLETE-RESOURCE-LIMIT`); `--json` = objek `myc.limits.v1` | **NON-blocking penuh** (limits = laporan, verdict target tidak pernah berubah); debt tipe resource_limit muncul di jalur nyata (mis. driver combinatorial budget memotong kasus → verdict tetap OK, `--require-complete` menaikkan INCONCLUSIVE pola 9.10); flag tak dikenal = fail-fast exit 2; jumlah entri adl FREEZE (blok 21 `_audit018.sh` "29 resource limit terdefinisi") — menambah entri LIMITS[] WAJIB update cek tsb |
...
| scenario            | `--scenario NAME [--scenario-file PATH]` | JSON profile (strict-validated) | **resep verifikasi per domain** (C5): one command activates the right gate recipe — `cli-daily` (run+analyzer), `library` (driver+exhaustive), `parser` (fuzz+run), `firmware` (freestanding+stack+divergence); `myc scenario list` / `info`; user profiles via `scenarios.json` | `--scenario auto` (D3) infers the smallest sufficient recipe from source structure (firmware `volatile`/ISR/packed → firmware; loop baca stdin `fgets`/`read` + parsing `strtol`/`sscanf`/`isdigit` → parser; `//@` contract → library; `main()` → cli-daily — IDE-5) and reports **why**; **DS-12 env contract** (`stack_budget`, `no_heap`, `no_recursion`, …) recorded in the report; never changes the verdict (all gates stay optional) |
...
| eig                  | `myc eig <file> [--profile <id>] [--budget-ms N] [--unchanged] [--json]`; `myc check <file> --eig-apply [--budget-ms N]` | derivasi murni: frontier + observasi + ledger `.myc/calibration.json` + profil `.myc/profiles/<id>.json` (Fase 7, #2029/DS-14); apply = opt-in di dalam `myc check` | **Expected-Information-Gain scheduler**: rekomendasi eksperimen terurut skor `expected_value = P(new_evidence) × severity × scope / (time_cost × token_cost)`; prior = tabel deterministik yang dikalibrasi dari Trust Calibration Ledger (rule `eig-<slug-hazard>`, accepted/confirmed_later naik, rejected/harmful_fix turun) + profil SOL-20 (kelas gate historis harness); `--budget-ms N` → flag `within_budget` per rekomendasi; `--unchanged` → prior dibagi dua (informasi baru kecil). **`--eig-apply`**: setelah L1 bersih, jalankan **paling banyak satu** eksperimen `within_budget` (default budget 5000 ms bila `--budget-ms` absen); masuk cache key g2; default **OFF** (tidak mengubah `myc check` biasa). MCP `next` = rekomendasi tanpa apply. | **observation, NON-blocking** kecuali `--eig-apply` yang menjalankan **satu** gate terpilih (hasil gate itu mengikuti aturan gate tersebut); `myc eig` sendiri tidak mengubah verdict; ledger/profil gagal baca = prior tabel murni; tanpa `--eig-apply`, `myc eig` tetap lapisan PERENCANAAN saja |
| parallel-gates       | `--parallel-gates` (bersama `--analyze --run`) | spawn `--run` berbarengan dengan `--analyze` setelah compile clean (P4) | **Overlap gate independen**: worker menulis ke `myc_result` terpisah, join, lalu `myc_gate_set_status` RUNTIME dalam urutan kanonik (setelah analyzer/checked/prove/filc). Analyzer findings = join + buang hasil run (receipt sama dengan default OFF). Timeout anak tetap `timeout_ms`. Masuk cache key g2. Default **OFF**. | Tidak mengubah verdict/receipt pada jalur sukses vs sekuensial; fail-closed ke sekuensial bila thread gagal di-spawn |
| production           | `--production` | require-complete + floor versi backend (P12, P5-T03, INV-015) | **Production mode**: `require_complete=1`; toolchain di bawah `min_version` tidak boleh CLEAN (UNAVAILABLE + debt). FINDINGS tidak ditimpa. Masuk cache key g2. Default **OFF**. | Verdict bisa INCONCLUSIVE bila gap |
...
| --- | --- | --- | --- |
| (default ON) | `.myc/evidence_cache.json` | **Incremental evidence cache**: run dengan (source, scenario, tool, cwd, stdin, timeout, output cap, header dir, resep gate Fase 5/6) yang sama dengan run sebelumnya di-**REPLAY** tanpa menjalankan backend (verdict/gates/debt/diags/counts/receipt identik SOL-18); miss + source berubah → **delta report** (fungsi berubah + dependents). Proses MCP hidup: skip parse JSON hanya bila sha256 byte file == sidecar == memo (G2). | **NON-blocking**: `.myc/` tak terbaca = miss, jalur normal; replay TIDAK pernah mengubah verdict; key v2 (spesifikasi per dimensi: `docs/cache-key.md`, PR-011); memo in-process tidak melewati PR-013 (sidecar mismatch = ignore + recompute) |
...
| `--watch-diff` (alias `--delta`) | — | **Delta assurance per-fungsi** (IDE-6, T5): bandingkan source vs baseline cache (scenario sama) → status per-fungsi `berubah / identik / baru / hilang / dependent` (fungsi identik yang memanggil fungsi berubah) + timing delta; teks `watch-diff:` + JSON `watch_diff` (baseline, counts, funcs[]); run pertama tanpa baseline → jujur "belum ada baseline"; alias `--delta` = perintah yang sudah disarankan `myc prompt`. **Lite (`--lite` / MCP `verify`)**: bila delta ada dan fungsi finding **tidak** `CHANGED`, `action=STOP_COMPILE_CLEAN` dengan why `fungsi finding tidak berubah` (anti-churn inner loop; mengalahkan eskalasi runtime default). | **NON-blocking penuh** pada verdict (hasil gate TIDAK berubah); TIDAK masuk scenario hash (output-only, seperti `--json-summary`); MCP `check` menerima flag `--watch-diff`/`--delta`; fast inner loop untuk agent yang mengedit per-fungsi dalam satu sesi |
