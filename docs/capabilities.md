...
| `--watch-diff` (alias `--delta`) | — | **Delta assurance per-fungsi** (IDE-6, T5): bandingkan source vs baseline cache (scenario sama) → status per-fungsi `berubah / identik / baru / hilang / dependent` (fungsi identik yang memanggil fungsi berubah) + timing delta; teks `watch-diff:` + JSON `watch_diff` (baseline, counts, funcs[]); run pertama tanpa baseline → jujur "belum ada baseline"; alias `--delta` = perintah yang sudah disarankan `myc prompt` | **NON-blocking penuh** (verdict/hasil TIDAK berubah); TIDAK masuk scenario hash (output-only, seperti `--json-summary`); MCP `check` menerima flag `--watch-diff`/`--delta`; fast inner loop untuk agent yang mengedit per-fungsi dalam satu sesi |
