...
| MYC-AUDIT-019 | Medium | `myc.c`/CLI + test POSIX | CLI unknown flag & arg tanpa nilai tidak fail-fast (OOB baca argv); portabilitas test POSIX | ✅ SELESAI 2026-08-03 (CLI fail-fast unknown flag → `myc: unknown option` + exit 2; `--run-stdin`/`--cwd` tanpa nilai → exit 2; parsing flag ber-`known`; `strdup`→`myc_strdup` POSIX; `WIFSIGNALED` sanitizer → `ok=1`; `-Werror` portabel; `_audit018.sh` 6/6 + Lampiran A 27/28) |
| MYC-AUDIT-020 | Medium | `myc.c`/API/MCP | `--timeout`/`--output-cap` & validasi request tidak ada (negatif/overflow lolos) | ✅ SELESAI 2026-08-03 (`--timeout` 0–600000, `--output-cap` 0–100 MiB, cwd non-kosong; negatif/overflow/non-angka fail-fast exit 2; berlaku CLI+API+MCP via `myc_request_validate`) |
| MYC-AUDIT-021 | Medium | `filc.c`/`proc.c` | run_stdin ke Fil-C di WSL tidak diteruskan (env Windows tak sampai WSL) | ✅ SELESAI 2026-08-03 (Lampiran A 28/28: `WSLENV=...:MYC_FILC_STDIN/p` path translation; template `$MYC_FILC_STDIN` + fallback `wslpath`; file stdin temp di-`remove`; fixture `ok_filc_stdin.c`) |
| MYC-AUDIT-022 | Medium | `compile.c`/`proc.c`/CLI | Diagnostic teks sulit di-parse mesin; versi toolchain tidak persis | ✅ SELESAI 2026-08-03 (`-fdiagnostics-format=json` + `ingest_gcc_diagnostics`; `myc_tool_version()` → `gcc_version`/`clang_version` teks+JSON; `myc version` cetak versi persis) |
| MYC-AUDIT-023 | Medium | CI/build/test | Belum ada CI lintas-platform (Windows + Linux) | ✅ SELESAI 2026-08-03 (Fase 8: `build.sh`, `test/_ci_linux.sh`, `.github/workflows/ci.yml` job windows+linux; self-dogfooding + fixture kunci) |
...
| MYC-AUDIT-027 | Medium | `driver.c` | Driver tanpa case record/replay, boundary portfolio, & combinatorial budget | ✅ SELESAI 2026-08-04 (`myc_driver_case` per kasus — case_id/func/param values/alloc sizes/status run-skip; `driver_case_records[]` (maks 256) teks+JSON+capsule; harness `setvbuf(_IONBF)` agar marker per-case ter-flush walau sanitizer abort; `harness_sha256`; `build_combos` full vs coverage-first ≤32; fixture `ok_driver_bounded.c`) |
| MYC-AUDIT-028 | Medium | `myc.c` | File/stdin dibaca penuh sebelum size cap (file raksasa → OOM) | ✅ SELESAI 2026-08-04 (`read_file_capped`/`read_stdin_capped` baca bertahap & tolak segera saat cap terlampaui; source 1 MiB, `--run-stdin` `MYC_MAX_STDIN_BYTES` 8 MiB; CLI fail-fast exit 2 + pesan cap; `myc_request_validate` cap `run_stdin_len` → `MYC_ERR_STDIN_TOO_LARGE`) |
| MYC-AUDIT-029 | Medium | `myc.c` | `source`/`file_path` tersebar; pembacaan duplikat antar CLI/API/MCP | ✅ SELESAI 2026-08-04 (`myc_source_input` formal struct + enum `myc_source_kind` MEMORY/FILE/STDIN; loader terpusat `myc_source_load()` cap+NUL policy+error typed; pipeline HANYA in-memory; CLI & MCP pakai loader sama dengan API) |
| MYC-AUDIT-030 | Medium | `myc.c` | File path tidak di-canonicalize (ejaan beda → fingerprint beda) | ✅ SELESAI 2026-08-04 (`myc_absolutize()` lexical — absolutize relatif via `my_getcwd`, normalisasi `\`→`/`, resolve `.`/`..` tanpa menyentuh filesystem; `myc_run()` canonicalisasi `req->cwd` ke salinan efektif tanpa mutasi caller; ejaan ekuivalen → fingerprint identik) |
...
- [x] long fingerprint material cannot OOB. (audit_lampiran T8 `run_fp_long_test`: cwd 3000-char, fingerprint 64-hex + deterministik; varian ASan di `_audit018.sh`; fix MYC-AUDIT-005)
- [x] `file_path`-only request. (audit_lampiran T9: `req.file_path` tanpa `source` diproses tanpa NULL deref, source di-load & di-hash; fix MYC-AUDIT-007)
...
- [x] OOM in JSON object insertion. (fase JSON di oom_alloc.c: json_new_obj/obj_set/arr_push/serialize 49 fail point tanpa crash, guard sb_reserve/json_obj_set/json_arr_push; MYC-AUDIT-009)
