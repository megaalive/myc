...
/* Jalankan satu source (buffer memori) dengan gate kind (seed PRNG
 * untuk fuzz). return 1 = violation, 0 = clean/OK, -1 = error. */
static int regress_run_src(const char *src, size_t len, const char *kind,
...
/* Jalankan satu source (dari file) dengan gate kind. return 1 =
 * violation, 0 = clean/OK, -1 = error. */
                           ...
                           unsigned seed, char *status_out, size_t status_cap)
{
    ...
    rc = regress_run_src(src, (size_t)sz, kind, seed, status_out,
                         status_cap);
    myc_free(src);
    return rc;
}
 ...
 * tiap seed input (kind + seed PRNG dari index) dijalankan pada source
 * yang diberikan — sama persis dengan myc_regress_run(out, file) tapi
 * tanpa file sementara. Mengisi *total, *resolved (OK), *failing;
 * return jumlah seed yang masih gagal. NON-blocking. */
                           ...
                           int *total, int *resolved, int *failing)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%31s %15s %127s %u", kind, hash, detail,
                   &seed) < 2)
            continue;
        rc = regress_run_src(source, len, kind, seed, status,
                             sizeof(status));
        t++;
        if (rc == 0)
            r++;
