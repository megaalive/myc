/*
 * driver.c -- Gate driver-generator (D2.2, --driver) + compare (A4).
...
static _Thread_local unsigned cmp_rng_state = CMP_PRNG_SEED;
                             ...
                             const drv_func *fn)
{
    ...
    if (total > CMP_MAX_CASES)
        total = CMP_MAX_CASES;
    cf->ncases = (int)total;
    cf->cases = (long *)myc_malloc(sizeof(long) * (size_t)cf->ncases *
                               (size_t)cf->nint);
    if (!cf->cases) {
        cf->ncases = 0;
        return 0;
