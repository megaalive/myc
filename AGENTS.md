...
| `persist.c` | Atomic .myc state writes (Batch PR-012, P3-T03): helper bersama `myc_persist_atomic_write` — tulis temp `<path>.tmp.<pid>` + flush + fsync/_commit + rename/replace atomik (MoveFileExA Windows) + parent-dir fsync POSIX + cleanup temp stale; crash kapan pun = OLD valid ATAU NEW valid (tidak pernah setengah); dipakai semua penulis state .myc (cache/ledger/assumptions/calibration/exhaustive/profiles/regression seed); NON-blocking penuh |
| `limit.c` / `limit.h` | Resource ceilings (PR-018, P7-T01): `LIMITS[]` tabel batas input/runtime deterministik (source/stdin/args/duration/driver-combos/output/gates), audit `myc limits --require-complete`; kegagalan batas = INCONCLUSIVE + debt; NON-blocking penuh — verdict tidak pernah turun kecuali hard gate memilihnya |
| `alloc.c` / `alloc.h` | Allocator wrapper FORMAL (PR-019, P7-T02): produksi passthrough ke libc (`myc_malloc/calloc/realloc/free`); build `-DMYC_ALLOC_TEST` mengaktifkan hook `myc_alloc_set_fail_after(N)` (N alokasi pertama sukses, sisanya NULL) + `myc_alloc_fail_count`/`myc_alloc_call_count` untuk OOM injection tanpa GNU ld `--wrap`; SEMUA alokasi source myc wajib lewat wrapper ini (block 22 audit018 memverifikasi tak ada panggilan mentah) |
