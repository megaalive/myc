...
gcc -O2 -std=c11 -Wall -Wextra -I. -DMYC_NO_MAIN -o test\stress_threads.exe test\stress_threads.c myc.c proc.c scanner.c policy.c compile.c report.c sha256.c lint.c run.c sanloc.c contract.c prove.c filc.c driver.c json.c gate.c negative.c taxonomy.c alloc.c >nul 2>&1
...
if exist test\sanloc_test.exe (
  test\sanloc_test.exe | findstr "FAIL=0" >nul && echo [OK] sanloc_test lulus IDE-1 location extractor || echo [FAIL] sanloc_test menemukan kegagalan
) else (
...
del test\lite_agent_test.exe 2>nul
if exist test\.replay_tmp rmdir /s /q test\.replay_tmp
if exist .myc\regression rmdir /s /q .myc\regression
...
for %%f in (myc.c proc.c scanner.c policy.c compile.c report.c sha256.c lint.c run.c sanloc.c contract.c state.c abi.c resource.c units.c profile.c calibrate.c eig.c candidate.c prove.c filc.c driver.c json.c mcp.c negative.c agent.c witness.c ledger.c transaction.c frontier.c observation.c causal.c nextbest.c cache.c context.c budget.c assume.c taxonomy.c prompt.c stack.c mutate.c scenario.c matrix.c canary.c testaudit.c perturb.c concur.c regress.c persist.c limit.c alloc.c) do (
  echo === %%f
  myc.exe check "%%f" > %OUT%
...
echo --- Fase 5 (SOL-12): Resource Linearity Ledger (//@ resource)
set OUT=%TEMP%\myc_rsrc_out.txt
myc.exe resource test\fixtures\resource_clean.c > %OUT% 2>&1
findstr /C:"2 acquire, 2 release" %OUT% >nul && echo [OK] resource: fixture bersih seimbang (2 acquire, 2 release) || echo [FAIL] resource: fixture bersih tidak seimbang
findstr /C:"0 leak, 0 double, 0 unknown" %OUT% >nul && echo [OK] resource: fixture bersih 0 temuan || echo [FAIL] resource: fixture bersih memunculkan temuan
myc.exe resource test\fixtures\resource_leak.c > %OUT% 2>&1
findstr /C:"1 leak" %OUT% >nul && echo [OK] resource: leak terdeteksi (fopen tanpa fclose) || echo [FAIL] resource: leak tidak terdeteksi
findstr /C:"witness: acq@11..end@15" %OUT% >nul && echo [OK] resource: witness acq..end benar || echo [FAIL] resource: witness acq..end salah
myc.exe resource test\fixtures\resource_double.c > %OUT% 2>&1
findstr /C:"1 double" %OUT% >nul && echo [OK] resource: double-release terdeteksi || echo [FAIL] resource: double-release tidak terdeteksi
myc.exe resource test\fixtures\resource_transfer.c > %OUT% 2>&1
findstr /C:"transfer=1" %OUT% >nul && echo [OK] resource: resource ditransfer (return var) bukan leak || echo [FAIL] resource: resource transfer tidak dikenali
myc.exe check test\fixtures\resource_leak.c --no-cache --json-summary > %OUT% 2>&1
findstr /C:"\"verdict\":\"OK\"" %OUT% >nul && echo [OK] resource: temuan TIDAK menurunkan verdict (NON-blocking) || echo [FAIL] resource: verdict berubah oleh observasi ledger
findstr /C:"\"resource\":{\"ran\":true" %OUT% >nul && echo [OK] resource: resource masuk JSON summary || echo [FAIL] resource: tidak masuk JSON summary
del %OUT%
echo --- Fase 5 (SOL-11): Units / Shape / Provenance Contracts (//@ unit)
set OUT=%TEMP%\myc_units_out.txt
myc.exe units test\fixtures\units_clean.c > %OUT% 2>&1
findstr /C:"0 unbound, 0 unit-mismatch, 0 shape-dim, 0 dup-conflict" %OUT% >nul && echo [OK] units: fixture bersih 0 temuan || echo [FAIL] units: fixture bersih memunculkan temuan
myc.exe units test\fixtures\units_broken.c > %OUT% 2>&1
findstr /C:"1 unbound" %OUT% >nul && echo [OK] units: unbound identifier terdeteksi || echo [FAIL] units: unbound tidak terdeteksi
findstr /C:"1 unit-mismatch" %OUT% >nul && echo [OK] units: unit mismatch pada assignment || echo [FAIL] units: unit mismatch tidak terdeteksi
findstr /C:"2 shape-dim" %OUT% >nul && echo [OK] units: shape-dim capacity vs length || echo [FAIL] units: shape_dim tidak terdeteksi
findstr /C:"2 dup-conflict" %OUT% >nul && echo [OK] units: dup annotation ganda || echo [FAIL] units: dup tidak terdeteksi
myc.exe check test\fixtures\units_broken.c --no-cache --json-summary > %OUT% 2>&1
findstr /C:"\"verdict\":\"OK\"" %OUT% >nul && echo [OK] units: temuan TIDAK menurunkan verdict (NON-blocking) || echo [FAIL] units: verdict berubah oleh observasi units
myc.exe check test\fixtures\units_clean.c --no-cache --json-summary > %OUT% 2>&1
findstr /C:"\"units\":{\"ran\":true" %OUT% >nul && echo [OK] units: units masuk JSON summary || echo [FAIL] units: tidak masuk JSON summary
del %OUT%
echo --- Fase 7 (SOL-20): Model/Harness Error Fingerprint
set OUT=%TEMP%\myc_profile_out.txt
if exist .myc\profiles rmdir /s /q .myc\profiles
myc.exe check test\fixtures\units_clean.c --no-cache > %OUT% 2>&1
if not exist .myc\profiles (echo [OK] profile: tanpa opt-in, tidak ada profil dibuat) else (echo [FAIL] profile: profil dibuat tanpa opt-in)
if exist .myc\profiles rmdir /s /q .myc\profiles
myc.exe check test\fixtures\units_clean.c --no-cache --profile win-harness > %OUT% 2>&1
myc.exe profile show win-harness > %OUT% 2>&1
findstr /C:"id      : win-harness" %OUT% >nul && echo [OK] profile: tercatat via --profile || echo [FAIL] profile: tidak tercatat via --profile
myc.exe check test\fixtures\units_clean.c --no-cache --profile win-harness > %OUT% 2>&1
myc.exe profile show win-harness > %OUT% 2>&1
findstr /C:"checks  : 2" %OUT% >nul && echo [OK] profile: agregasi antar check bekerja || echo [FAIL] profile: agregasi tidak bekerja
set MYC_PROFILE_ID=win-env
myc.exe check test\fixtures\units_clean.c --no-cache > %OUT% 2>&1
set MYC_PROFILE_ID=
myc.exe profile show win-env > %OUT% 2>&1
findstr /C:"checks  : 1" %OUT% >nul && echo [OK] profile: env MYC_PROFILE_ID dihormati || echo [FAIL] profile: env tidak dihormati
myc.exe check test\fixtures\units_broken.c --no-cache --json-summary > %OUT% 2>&1
findstr /C:"\"verdict\":\"OK\"" %OUT% >nul && echo [OK] profile: NON-blocking (verdict tetap) || echo [FAIL] profile: verdict berubah oleh profil
...
:: id invalid = fail-fast.
myc.exe calibrate mark "bad id" accepted >nul 2>&1
if errorlevel 1 (echo [OK] calibrate: id invalid ditolak) else (echo [FAIL] calibrate: id invalid tidak fail-fast)
...
if exist .myc\calibration.json del /q .myc\calibration.json
del %OUT%
echo --- 6o. Fase 7 (#2029, DS-14): Expected-Information-Gain scheduler (myc eig)
set OUT=%TEMP%\myc_eig_out.txt
if exist .myc\calibration.json del /q .myc\calibration.json
if exist .myc\profiles rmdir /s /q .myc\profiles
myc.exe eig test\fixtures\eig_clean.c > %OUT% 2>&1
findstr /C:"eig scheduler (Fase 7, DS-14)" %OUT% >nul && echo [OK] eig: laporan scheduler tercetak || echo [FAIL] eig: laporan scheduler tidak tercetak
findstr /C:"rekomendasi: 6" %OUT% >nul && echo [OK] eig: 6 rekomendasi dari 6 hazard untested || echo [FAIL] eig: jumlah rekomendasi bukan 6
findstr /C:"expected-value=" %OUT% >nul && echo [OK] eig: expected-value tercetak per rekomendasi || echo [FAIL] eig: expected-value tidak tercetak
myc.exe eig test\fixtures\eig_clean.c --unchanged > %OUT% 2>&1
findstr /C:"source_changed: 0" %OUT% >nul && echo [OK] eig: --unchanged dicerminkan || echo [FAIL] eig: --unchanged tidak dicerminkan
myc.exe eig test\fixtures\eig_clean.c --budget-ms 100 > %OUT% 2>&1
findstr /C:"within_budget: 0" %OUT% >nul && echo [OK] eig: budget 100ms gives within_budget 0 || echo [FAIL] eig: budget 100ms tidak menghasilkan within_budget 0
myc.exe calibrate mark eig-runtime-memory-asan-ubsan accepted --match "eig_test" >nul 2>&1
myc.exe calibrate mark eig-runtime-memory-asan-ubsan accepted --match "eig_test" >nul 2>&1
myc.exe calibrate mark eig-runtime-memory-asan-ubsan accepted --match "eig_test" >nul 2>&1
myc.exe eig test\fixtures\eig_clean.c > %OUT% 2>&1
findstr /C:"calibrated_rules: 1" %OUT% >nul && echo [OK] eig: kalibrasi ledger SOL-21 terbaca || echo [FAIL] eig: kalibrasi ledger tidak terbaca
myc.exe calibrate reset eig-runtime-memory-asan-ubsan >nul 2>&1
myc.exe check test\fixtures\eig_clean.c --no-cache --profile ci-eig >nul 2>&1
myc.exe eig test\fixtures\eig_clean.c --profile ci-eig > %OUT% 2>&1
findstr /C:"profile: ci-eig" %OUT% >nul && echo [OK] eig: profil SOL-20 dibaca || echo [FAIL] eig: profil SOL-20 tidak dibaca
myc.exe eig test\fixtures\eig_clean.c --json > %OUT% 2>&1
findstr /C:"\"expected_value\"" %OUT% >nul && echo [OK] eig: output --json memuat expected_value || echo [FAIL] eig: output --json tidak memuat expected_value
myc.exe check test\fixtures\eig_clean.c --no-cache > %OUT% 2>&1
findstr /C:"verdict:   OK" %OUT% >nul && echo [OK] eig: NON-blocking (verdict tetap OK) || echo [FAIL] eig: verdict berubah oleh eig
if exist .myc\calibration.json del /q .myc\calibration.json
if exist .myc\profiles rmdir /s /q .myc\profiles
del %OUT%
echo --- 6p. Fase 7 (SOL-10): Candidate Tournament dengan Pareto Frontier (myc compare-candidates)
set OUT=%TEMP%\myc_cand_out.txt
myc.exe compare-candidates test\fixtures\cand_base.c test\fixtures\cand_better.c test\fixtures\cand_worse.c > %OUT% 2>&1
if errorlevel 1 (echo [FAIL] cand: tournament harus NON-blocking exit 0) else (echo [OK] cand: tournament NON-blocking exit 0)
findstr /C:"candidate tournament (Fase 7, SOL-10)" %OUT% >nul && echo [OK] cand: laporan tournament tercetak || echo [FAIL] cand: laporan tournament tidak tercetak
findstr /C:"2 kandidat vs baseline" %OUT% >nul && echo [OK] cand: 2 kandidat vs baseline || echo [FAIL] cand: jumlah kandidat salah
findstr /C:"dimensi terukur: 8 dari 9" %OUT% >nul && echo [OK] cand: 8 dari 9 dimensi terukur || echo [FAIL] cand: dimensi terukur salah
findstr /C:"stack_impact = UNMEASURED" %OUT% >nul && echo [OK] cand: gap stack_impact terlihat || echo [FAIL] cand: gap stack_impact tidak terlihat
findstr /C:"findings=0 obligations_lost=0 churn=14" %OUT% >nul && echo [OK] cand: cand_better findings 0 + kontrak dipertahankan || echo [FAIL] cand: dimensi cand_better salah
findstr /C:"findings=2 obligations_lost=1 churn=24" %OUT% >nul && echo [OK] cand: cand_worse findings 2 + obligations_lost 1 || echo [FAIL] cand: dimensi cand_worse salah
findstr /C:"didominasi oleh: test\fixtures\cand_base.c" %OUT% >nul && echo [OK] cand: cand_worse didominasi oleh baseline || echo [FAIL] cand: dominator cand_worse bukan baseline
myc.exe compare-candidates test\fixtures\cand_base.c test\fixtures\cand_better.c test\fixtures\cand_worse.c --json > %OUT% 2>&1
findstr /C:"myc.candidate.v1" %OUT% >nul && echo [OK] cand: output --json memuat schema myc.candidate.v1 || echo [FAIL] cand: output --json tanpa schema
